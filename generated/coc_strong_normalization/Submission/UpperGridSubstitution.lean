import Submission.UpperGridTransport
...
theorem upperGridSemantics_var (hδ : FirstGridContext (p + 1) Γ ρ δ)
    (hw : BoundedWf (p + 4) Γ) (hj : Γ[j]? = some A)
    (hρ : ShapeContext (.type (p + 2)) Γ ρ) :
    (upperGridSemantics p Γ ρ δ (.var j)).1 = δ j :=
  TowerValue.force_eq _ (hδ.var_code hw hj hρ)

theorem upperShapeEval_substitute_related (h : BoundedTyping (p + 4) Γ t T)
    ...
    shapeEval (p + 4) (p + 2) Δ ρ' (sub σ t) = shapeEval (p + 4) (p + 2) Γ ρ t :=
  (shapeEval_substitute h hw hσ hk rfl).trans (shapeEval_env h heρ)

theorem firstGridShape_substitute_related (h : BoundedTyping (p + 4) Γ A (.srt s))
    (hw : BoundedWf (p + 4) Δ) (hσ : BoundedSubCtx (p + 4) Γ Δ σ)
    (hk : ∀ j, kindAt (.type (p + 2)) (σ j) = false)
    (heρ : ∀ j C, Γ[j]? = some C → shapeEval (p + 4) (p + 2) Δ ρ' (σ j) = ρ j) :
    firstGridShape (p + 1) Δ ρ' (sub σ A) v = firstGridShape (p + 1) Γ ρ A v :=
  (firstGridShape_substitute h hw hσ hk).trans (firstGridShape_env h heρ)

theorem upperShapeSubstitute_related_up (hA : BoundedTyping (p + 4) Γ A (.srt s))
    (hw : BoundedWf (p + 4) Δ) (hσ : BoundedSubCtx (p + 4) Γ Δ σ)
    (heρ : ∀ j C, Γ[j]? = some C → shapeEval (p + 4) (p + 2) Δ ρ' (σ j) = ρ j) :
    ∀ j C, (A :: Γ)[j]? = some C →
      shapeEval (p + 4) (p + 2) (sub σ A :: Δ) (push x ρ') (upSub σ j) = push x ρ j := by
  intro j C hj
  cases j with
  | zero => rfl
  | succ j =>
...
theorem upperSubstitute_related_up (hA : BoundedTyping (p + 4) Γ A (.srt s))
    (hw : BoundedWf (p + 4) Δ) (hσ : BoundedSubCtx (p + 4) Γ Δ σ)
    (hρ' : ShapeContext (.type (p + 2)) Δ ρ') (hδ' : FirstGridContext (p + 1) Δ ρ' δ')
    (heδ : ∀ j C, Γ[j]? = some C → (upperGridSemantics p Δ ρ' δ' (σ j)).1 = δ j)
    (hx : x.code = arityAt (.type (p + 2)) (sub σ A))
    (hy : y.code = firstGridShape (p + 1) Δ ρ' (sub σ A) x) :
    ∀ j C, (A :: Γ)[j]? = some C →
      (upperGridSemantics p (sub σ A :: Δ) (push x ρ') (push y δ') (upSub σ j)).1 =
        push y δ j := by
  have hA' := hA.substitute hw hσ
  intro j C hj
  cases j with
  | zero =>
    exact upperGridSemantics_var (hδ'.up hA' hy) (.cons hw hA') rfl (hρ'.up hx)
  | succ j =>
    exact ((upperGridSemantics_ren (hσ j C hj) (.cons hw hA') (.weaken Δ (sub σ A))
...
theorem upperGridSemantics_substitute_related (h : BoundedTyping (p + 4) Γ t T)
    (hw : BoundedWf (p + 4) Δ) (hσ : BoundedSubCtx (p + 4) Γ Δ σ)
    (hk : ∀ j, kindAt (.type (p + 2)) (σ j) = false)
    ...
    (heρ : ∀ j C, Γ[j]? = some C → shapeEval (p + 4) (p + 2) Δ ρ' (σ j) = ρ j)
    (heδ : ∀ j C, Γ[j]? = some C → (upperGridSemantics p Δ ρ' δ' (σ j)).1 = δ j) :
    (upperGridSemantics p Δ ρ' δ' (sub σ t)).1 = (upperGridSemantics p Γ ρ δ t).1 ∧
    (∀ s, BoundedTyping (p + 4) Γ t (.srt s) →
      (upperGridSemantics p Δ ρ' δ' (sub σ t)).2 = (upperGridSemantics p Γ ρ δ t).2) := by
  induction t generalizing Γ T Δ σ ρ ρ' δ δ' with
  | var j =>
    obtain ⟨A, hj, hc⟩ := h.generation
    have hv : (upperGridSemantics p Δ ρ' δ' (sub σ (.var j))).1 =
        (upperGridSemantics p Γ ρ δ (.var j)).1 := by
      rw [upperGridSemantics_var hδ h.wf hj hρ]
      exact heδ j A hj
    refine ⟨hv, ?_⟩
    intro s hs
    funext u f
    rw [upperGridSemantics_decode (hs.substitute hw hσ) hs.var_below_top hρ',
  ...
  | srt s =>
    ...
    rw [upperGridSemantics_sort_value (ht.substitute hw hσ) hρ',
      upperGridSemantics_sort_value ht hρ]
  ...
  | app g a ihg iha =>
    ...
    have hA' := hA.substitute hw hσ
    have hv : (upperGridSemantics p Δ ρ' δ' (sub σ (.app g a))).1 =
        (upperGridSemantics p Γ ρ δ (.app g a)).1 := by
      change (upperGridSemantics p Δ ρ' δ' (.app (sub σ g) (sub σ a))).1 = _
      rw [upperGridSemantics_app (hg.substitute hw hσ) (ha.substitute hw hσ) hρ',
      ...
      apply firstGridApply_congr (arityAt_sub hk A)
      · intro x hx
        exact firstGridShape_substitute_related hA hw hσ hk heρ
      · intro x hx z
    ...
    intro s hs
    funext u f
    rw [upperGridSemantics_decode (hs.substitute hw hσ) hs.app_below_top hρ',
  ...
  | lam A b ihA ihb =>
    ...
    have hA' := hA.substitute hw hσ
    refine ⟨?_, ?_⟩
    · change (upperGridSemantics p Δ ρ' δ' (.lam (sub σ A) (sub (upSub σ) b))).1 = _
      rw [upperGridSemantics_lam (hPi.substitute hw hσ)
          (hb.substitute (.cons hw hA') (hσ.up hw hA')) hρ',
        upperGridSemantics_lam hPi hb hρ,
        ← show sub σ (.lam A b) = .lam (sub σ A) (sub (upSub σ) b) from rfl,
        upperShapeEval_substitute_related (.lam hPi hb hs) hw hσ hk heρ]
      apply firstGridLambda_congr (arityAt_sub hk A)
      · intro x hx
        exact firstGridShape_substitute_related hA hw hσ hk heρ
      · intro x hx z
        exact firstGridShape_substitute_related hB (.cons hw hA') (hσ.up hw hA')
      ...
      · intro x hx y hy
        exact (ihb hb (.cons hw hA') (hσ.up hw hA') (kindAt_upSub_false hk)
          ...
          (upperSubstitute_related_up hA hw hσ hρ' hδ' heδ hx hy)).1
    · intro r hsort
  ...
  | pi A B ihA ihB =>
    ...
    have hA' := hA.substitute hw hσ
    have he : upperGridSemantics p Δ ρ' δ' (sub σ (.pi A B)) =
        upperGridSemantics p Γ ρ δ (.pi A B) := by
      apply upperGridSemantics_pi_compare (ht.substitute hw hσ) ht hρ' hρ
      ...
      · intro x hx y hy
        exact (ihB hB (.cons hw hA') (hσ.up hw hA') (kindAt_upSub_false hk)
          ...
          (hδ.up hA (hy.trans (firstGridShape_substitute_related hA hw hσ hk heρ)))
          (hδ'.up hA' hy) (upperShapeSubstitute_related_up hA hw hσ heρ)
          (upperSubstitute_related_up hA hw hσ hρ' hδ' heδ hx hy)).2 sB hB
    exact ⟨congrArg Prod.fst he, fun _ _ => congrArg Prod.snd he⟩

/-- Single substitution preserves the fiber value of every typed term. -/
theorem upperGridSemantics_subst_value (hb : BoundedTyping (p + 4) (A :: Γ) b B)
    (ha : BoundedTyping (p + 4) Γ a A) (hA : BoundedTyping (p + 4) Γ A (.srt sA))
    (hρ : ShapeContext (.type (p + 2)) Γ ρ) (hδ : FirstGridContext (p + 1) Γ ρ δ) :
    (upperGridSemantics p Γ ρ δ (subst 0 a b)).1 =
      (upperGridSemantics p (A :: Γ) (push (shapeEval (p + 4) (p + 2) Γ ρ a) ρ)
        (push (upperGridSemantics p Γ ρ δ a).1 δ) b).1 := by
  rw [subst_eq_sub]
  apply (upperGridSemantics_substitute_related hb ha.wf (.single ha) ?_
    (hρ.up (shapeEval_typed_code ha rfl hρ)) hρ
    (hδ.up hA (upperGridSemantics_typed_code ha hρ)) hδ ?_ ?_).1
  · intro j
    cases j with
...
/-- Carrier substitution at every bound `p + 4`, including the top sort. -/
theorem upperGridSemantics_subst_carrier (hB : BoundedTyping (p + 4) (A :: Γ) B (.srt sB))
    (ha : BoundedTyping (p + 4) Γ a A) (hA : BoundedTyping (p + 4) Γ A (.srt sA))
    (hρ : ShapeContext (.type (p + 2)) Γ ρ) (hδ : FirstGridContext (p + 1) Γ ρ δ) :
    (upperGridSemantics p Γ ρ δ (subst 0 a B)).2 =
      (upperGridSemantics p (A :: Γ) (push (shapeEval (p + 4) (p + 2) Γ ρ a) ρ)
        (push (upperGridSemantics p Γ ρ δ a).1 δ) B).2 := by
  rw [subst_eq_sub]
  apply (upperGridSemantics_substitute_related hB ha.wf (.single ha) ?_
    (hρ.up (shapeEval_typed_code ha rfl hρ)) hρ
    (hδ.up hA (upperGridSemantics_typed_code ha hρ)) hδ ?_ ?_).2 sB hB
  · intro j
    cases j with
    | zero => exact ha.not_kindAt hA rfl
    | succ j => rfl
